module.exports = {
  ...
  codehub: function(params) {
    let url = ''
    let title = ''
    let lines = []

    if (params.object_kind == 'push') {
      const commit = params.commits

      url = commit.url
      title = `【华为云】${params.user_name} 提交了代码到 ${params.project.namespace}/${params.project.name}`
      lines.push(title)
      lines.push(`【COMMIT ID】${commit.id}`)
      lines.push(`【COMMIT LOG】${commit.message}`)
      lines.push(`【OPERATOR】${params.user_name}`)
      ...
      const attr = params.object_attributes
      url = attr.url
      title = `【华为云】${params.user.name} 合并了 Merge Requset ${attr.title}`
      lines.push(title)
      lines.push(`【SOURCE】${attr.source_branch}`)
      lines.push(`【TARGET】${attr.target_branch}`)
      lines.push(`【OPERATOR】${params.user.name}`)
      ...
      url = 'https://gitlab.com'
      title = `【华为云】不支持的事件类型：${params.object_kind}`
      lines.push(title)
      lines.push(`如果需要，自己实现一下吧！`)
      console.log(JSON.stringify(params))
    }
